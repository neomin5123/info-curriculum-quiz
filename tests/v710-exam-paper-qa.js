...
assert(bank.questions.length===28,`question count ${bank.questions.length}`);
assert(bank.questions.filter(q=>q.points===4).length===21,'4pt count');
assert(bank.questions.filter(q=>q.points===2).length===7,'2pt count');
assert(bank.meta.rulesVersion==='1.6','rules version');
assert(html.includes('v7.1.0'),'version label');
assert(html.includes('28문항'),'UI count');
...
assert(totalUnits===98,`unit count ${totalUnits}`);
assert(fourPointStructured===21,`4pt structured ${fourPointStructured}/21`);
...
console.log(`v7.1 Exam Paper QA: OK (questions=${bank.questions.length}, 4pt=21, 2pt=7, units=${totalUnits}, multiUnitTasks=${multiUnitTasks})`);
