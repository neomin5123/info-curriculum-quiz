...
if(!bank||bank.questions?.length<150) throw new Error(`v613 bank regressed ${bank?.questions?.length}`);
if(Number(bank.meta?.questionCount||0)<150) throw new Error('meta count regressed');
...
if(units<311) throw new Error(`answer units regressed ${units}`);
const html=fs.readFileSync(path.join(root,'index.html'),'utf8');
if(!html.includes('180문항')||!html.includes('v6.17.0')) throw new Error('current UI/version missing');
console.log(`v6.13+ Exam Core regression QA: OK (questions=${bank.questions.length}, units=${units}, comparison=${compare}, multi=${multi}, pairs=${pairs.size})`);
