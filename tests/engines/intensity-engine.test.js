...
function raw(line){return (line.easy||[]).map((answer,i)=>({answer,gapId:line.gapIds?.easy?.[i]||`g${i}`})).filter(x=>x.answer&&line.text.includes(x.answer));}
for(const sk of order){
  ...
  for(const unit of Object.values(data[sk])) for(const [groupName,sections] of Object.entries(unit)) if(Array.isArray(sections)) for(const section of sections){
    ...
    for(const line of section.lines||[]){
      const source=raw(line); if(!source.length) continue;
      ...
      else {nonExactLines++;const profile=I.profileFor(section.title,groupName,tier);assert(selected.length<=Math.min(source.length,profile.maxCore),`${line.id}: max core`);}
