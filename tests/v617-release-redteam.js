...
if(sourceIds.size!==662||bank.questions.length!==180) throw new Error(`corpus/bank count ${sourceIds.size}/${bank.questions.length}`);
let units=0;
...
if(units!==371) throw new Error(`unit count ${units}`);
...
for(let seed=1;seed<=1000;seed++){
  ...
  if(new Set(qs.flatMap(q=>E.questionScopes(q).map(s=>s.subject))).size<5)throw new Error(`set subject cluster ${seed}`);
...
if(!html.includes('v6.17.0')||!sw.includes('curriloop-v6-17-0-20260920'))throw new Error('release version/cache mismatch');
for(const asset of ['/data/practice-bank.js?v=6.17.0','/js/practice-engine.js?v=6.17.0','/js/practice-ui.js?v=6.17.0','/app.js?v=6.17.0']) if(!html.includes(asset)||!sw.includes(asset))throw new Error(`release asset mismatch ${asset}`);
console.log('v6.17 final red team: OK (180q/371u, 1000 set generations, all filter combinations, 5000 adaptive updates)');
