...
assert(bank.questions.length===29,`question count ${bank.questions.length}`);
assert(bank.questions.filter(q=>q.points===4).length===22,'4pt count');
assert(bank.questions.filter(q=>q.points===2).length===7,'2pt count');
assert(bank.meta.rulesVersion==='1.6','rules version');
assert(html.includes('v7.2.0'),'version label');
assert(html.includes('29문항'),'UI count');
...
assert(totalUnits===102,`unit count ${totalUnits}`);
assert(fourPointStructured===22,`4pt structured ${fourPointStructured}/22`);
...
console.log(`v7.2 Exam Paper QA: OK (questions=${bank.questions.length}, 4pt=22, 2pt=7, units=${totalUnits}, multiUnitTasks=${multiUnitTasks})`);
